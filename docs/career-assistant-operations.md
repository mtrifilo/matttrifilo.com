...
| `groundedness` |    23 | Twelve questions answered from a document: the run must open one the test names (`assertReadsAnyOf`) and produce an answer that did not stop short except at the output cap (`assertAnswered`), and its `Sources:` trailer must name only documents in the server's read ledger, `metadata.readIds`, which also lists documents refused for size or budget (`assertCitesOnlyWhatItRead`), and may be absent only where the run opened that document (`assertCites`, the sixth case under "A red run"). The trailer is found by the page's own parser (`findSourcesTrailer` in `lib/chat/answer.ts`), so a line the page leaves on screen as prose is no trailer to the suite either, and a trailer on an answer carrying the decline sentence, or on no answer at all, fails `assertCites`. `evals/config.test.ts` refuses a citation test that does not carry all three of `assertCites`, `assertCitesOnlyWhatItRead` and `assertAnswered`. Eleven probes for plausible-but-absent facts must be declined rather than invented. Its pins follow the goldens' rule (see "Adding a golden"). |
...
What counts as a trailer is what the page hides: `findSourcesTrailer` in `lib/chat/answer.ts`, the parser the transcript uses, is the one the suites read. The trailer is the last line once the `Follow-ups:` block (whole or half written) is off the end, and it opens with `Sources:` after optional whitespace; text after the ids on that same line is read as ids. A bold, lower-case or upper-case label, a heading, or a citation line with more prose lines below it is not a trailer, because the page leaves each of those on screen, raw ids and all. A `Sources:` line naming no ids is hidden on the page and cites nothing, so the suites treat it as missing. So a row whose answer bolded its trailer is a missing trailer here, tolerated and counted under the rule above, and its ids are never compared with the ledger. A row reading `warning: no Sources: trailer`, or a red one reading `no Sources: trailer`, on an answer that plainly has a citation line is that case: read the line's form before blaming the model's citations. The other direction is closed: a trailer never rescues a decline or an empty answer, so `assertCites` fails an answer that carries the decline sentence (alone or with more around it, as the route itself judges a decline) or has no prose, whatever the trailer names. Every citation test also carries `assertAnswered`, so a run that stopped short for any reason but the output cap fails there with its trailer intact. An answer cut off at the output cap still counts as an answer; if its trailer was cut off with it, the row is tolerated and counted in `missingTrailer` like any other missing trailer.
