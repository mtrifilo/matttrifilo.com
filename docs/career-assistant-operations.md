...
- `activityRefusedUnknown` / `activityRefusedDuplicate` / `activityRefusedBudget` on the same line: checks turned away. Unknown means the model is guessing at repository ids; duplicate means it is looping, and counts a repeat answered from the call already in flight as well as one turned away (MTC-52); budget means a GitHub call was spent and its digest then dropped for want of tokens, which is the failure mode that costs a request and gives the visitor nothing.
...
**A duplicate check in one step is answered from the call already in flight** (2026-09-22, MTC-52), rather than refused: the second call awaits the first's promise and returns the same result, so one repository yields one outcome per step and a step can never carry both a digest and a failure for it. Two consequences are known and neither is fixed here. The digest is charged to the read budget once and delivered to each caller, so a repeated call resends a block already paid for. And `lib/chat/handler.ts` counts a check per non-refused tool output, so one fetch answered twice counts as two: a step that checks all three repositories and repeats one can leave the third repository without a progress row, though its digest still reaches the model and the answer is unaffected. The fix is in `toStep`'s cap prediction rather than in the tool, and the two comments there about a duplicate receiving `repository_already_checked` describe the behaviour before this change.
...
**The replayed part is not size-checked, on purpose** (decided 2026-09-22, MTC-65). The browser posts every earlier answer back with its progress part, and `lib/chat/validate.ts` skips that part without measuring it: every cap there measures text. Measured on 2026-09-22: the largest part the current corpus can produce (its three heaviest documents plus three GitHub checks) is 1,782 bytes, against 671 without headings; with the corpus's longest id and title and every heading at the caps it is 5,234 bytes. With every field at its cap, the test's worst case, it is 6,383 characters of JSON (up to three times that in bytes if the text is not ASCII), and the longest conversation the input budget accepts, carrying that part on every answer, is about 326 KB. Vercel accepts a 4.5 MB request body ([Vercel Functions limits](https://vercel.com/docs/functions/limitations)). A byte budget in validation was rejected because a wrong one would refuse a real visitor's next question, and it would not help against a tampered body: the body is parsed whole before validation, and the part is discarded unread like any other field the route ignores. Stripping headings from the outgoing request was not taken either: it saves about a kilobyte per turn with the current corpus and would change the client transport for that. `lib/chat/validate.test.ts` pins the 6,383 and holds that conversation under a tenth of Vercel's limit, so raising `MAX_HEADINGS`, `MAX_HEADING_CHARS`, `MAX_TITLE_CHARS`, the read budget or the activity budget fails a test and reopens this decision.
