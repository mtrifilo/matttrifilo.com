...
What counts as a trailer is what the page hides: `findSourcesTrailer` in `lib/chat/answer.ts`, the parser the transcript uses, is the one the suites read. A final line that opens with exactly `Sources:` is a trailer; a bold, lower-case or upper-case label, a heading, or a citation line with prose after it is not, because the page leaves each of those on screen, raw ids and all. So a row whose answer bolded its trailer is a missing trailer here, tolerated and counted under the rule above, and its ids are never compared with the ledger. A red row reading `no Sources: trailer` on an answer that plainly has a citation line is that case: read the line's form before blaming the model's citations. The other direction is closed: a trailer never rescues an answer, so `assertCites` fails the decline sentence or an answer with no prose whatever the trailer names, and every citation test also carries `assertAnswered`, so a run that stopped before finishing its answer fails there with its trailer intact.
