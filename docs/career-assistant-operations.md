...
**What a row shows.** A read row's label is the document's title (`Reading Résumé…`). Expanded, it carries two more lines: the corpus topic the document sits in, shown as a label (Résumé, Career, FAQ, Open source, Blog), and the document's `##` section titles joined in order. An activity row names the repository being checked on GitHub and carries neither. The writing row names nothing.

**Where the values come from.** All of them are looked up on the server at the moment the step is emitted, from the same index entry the read tool validates the model's id against. The model supplies an id and nothing else, so a row can only ever describe a document the index lists; an id that is not in the index earns no row at all. Headings are extracted at build time by `documentHeadings` in `lib/knowledge/build.ts` and live on the entry object, not in the index text the model reads: the index is in every request, so nothing is added to it for the sake of the view.

**Headings are titles only.** The tool hands the model a whole document, so the list is that document's outline, not a claim about which parts of it were used, and nothing underneath a heading travels with it. `MAX_HEADINGS` and `MAX_HEADING_CHARS` in `lib/chat/progress.ts` are the caps the wire is validated against; the corpus is held below both by `lib/knowledge/knowledge.test.ts`, so a document that grew past one would fail the suite rather than quietly show some of its sections and not the rest.
