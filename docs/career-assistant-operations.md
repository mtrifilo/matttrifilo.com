...
**What it leaves out, and why** (2026-09-22, MTC-52). Three things a repository publishes are dropped in `lib/chat/github-activity.ts` rather than asked for in the prompt, because they are accurate and they are noise to the hiring manager the assistant answers: a release whose tag ends in `-screenshots`, which in the psy-loop is a screenshot upload published as a full release and therefore what `/releases/latest` returns; a release whose tag does not open like a version (`v?\d+\.\d+`), which is the same hole the first rule leaves for an upload tag spelled some other way; and issue-tracker keys (`PSY-2080`, `MTC-52`) in pull request titles and commit subjects, with the punctuation they leave behind tidied up. A digest with no release simply has no release line. The known casualty of the key rule is any other word of that shape, `COVID-19` and `GPT-4` among them; sparing them would mean a list of acronyms with no end. The two activity goldens assert the absence of both at the other end of the pipe (`assertNoTicketKeys`, `assertNoScreenshotRelease`).
