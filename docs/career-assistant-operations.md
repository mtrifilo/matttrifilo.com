...
**The progress view counts a repeated repository once** (2026-09-22, MTC-63). Until then `lib/chat/handler.ts` counted a check per successful tool output, so one fetch answered twice counted as two against the progress-row cap, and a repository checked in a later step (two calls for one repository and one for another, then the third repository) was fetched with no row over it; its digest still reached the model and the answer was unaffected. The progress stage now counts the repositories whose digest reached the model, one per repository, as the session's own cap does. A step that checks all three repositories and repeats one was never affected: the SDK runs a step's tools only after the model finishes that step, so every call in it gets its row before any outcome arrives. `activityRefusedDuplicate` still counts the repeat, answered or refused, because a model repeating a check is looping either way.
