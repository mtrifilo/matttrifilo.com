...
| `groundedness` |    23 | Twelve questions answered from a document: the run must open one the test names (`assertReadsAnyOf`), and its `Sources:` trailer must name only documents the server's ledger shows were read (`assertCitesOnlyWhatItRead`) and may be absent only where the run opened that document (`assertCites`, the sixth case under "A red run"; `evals/config.test.ts` keeps the last two paired). Eleven probes for plausible-but-absent facts must be declined rather than invented.                                                                                                                                                                                              |
