...
| -------------- | ----: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `golden`       |   100 | Hiring-manager questions. The answer contains the distinctive facts, stays in the third person, and opened the document the fact lives in (`metadata.readIds`). One `llm-rubric` per test on top, graded three times inside an `assert-set` at `threshold: 0.6`, except the two MTC-45 activity goldens, which assert that the run checked the right repository (`metadata.activityRepos`), dated the work, and named no handle, because what a repository shipped this week is not a stable claim to grade.                                                                                                                                                                |
...
Every answer that is not a decline ends with a second trailer after `Sources:`: a line reading `Follow-ups:` and two or three questions under it, one to a line. The policy in `lib/chat/prompt.ts` asks for them and constrains them to questions the corpus can answer, ordered towards the outcomes Matt features first. The route parses that block out of the answer server-side, validates each proposal (a question mark at the end, 12 to 140 characters, no markdown, no link, no address, deduplicated, at most three) and puts the survivors on the message metadata as `followUps`; the browser strips both trailers out of the text and builds the pill row from the metadata alone, so a proposal that failed validation cannot appear even as prose. A decline, a cut-off answer and a run that never finished all carry none.

Seven goldens carry `assertFollowUpsAnswerable`, which is the only assertion in the suites that makes a second route call: it replays the golden's question and answer as `history` and asks the run's **first** proposal, exactly as the browser would, and fails if the assistant declines its own suggestion or proposes nothing at all. Only the first is asked, to bound what a golden costs. A red row there is a finding about the policy or the corpus, not an assertion to loosen: either the policy is inviting questions the documents cannot answer, or the document that would answer one is missing. Budget one extra route call for each of those seven tests on a full run.
