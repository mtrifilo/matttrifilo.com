 ...
 * /ask page and the /knowledge corpus pages are not rendered. It lives in
 * its own module with no imports beyond a type so the pages and the layout
 * can read it without pulling the validator, and through it the knowledge
 * index, into their module graph. Pages read `process.env` at render, which for a static page
