...
// the phone number back onto a public, indexed URL.
...
describe('public résumé PDF', () => {
  test('text extraction works (positive control)', () => {
    ...
    expect(text).toContain('matt.trifilo@gmail.com')
  })
  ...
  test('the only address is the public contact', () => {
    const addresses = [...new Set(text.match(/[\w.+-]+@[\w.-]+\.\w+/g) ?? [])]
    expect(addresses).toEqual(['matt.trifilo@gmail.com'])
  })

  test('raw bytes carry no tel: link either', () => {
    const raw = pdf.toString('latin1')
    expect(raw).not.toMatch(/tel:/i)
